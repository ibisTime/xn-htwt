...
public class BusDAOImpl extends AMybatisTemplate implements IBusDAO {
    ...
    @Override
    public void update(Bus data) {
        super.update(NAMESPACE.concat("update_bus"), data);
    }

    @Override
    public Bus select(Bus condition) {
        return super.select(NAMESPACE.concat("select_bus"), condition,
            Bus.class);
    }

    @Override
    public long selectTotalCount(Bus condition) {
        return super.selectTotalCount(NAMESPACE.concat("select_bus_count"),
            condition);
    }

    @Override
    public List<Bus> selectList(Bus condition) {
        return super.selectList(NAMESPACE.concat("select_bus"), condition,
            Bus.class);
    }

    @Override
    public List<Bus> selectList(Bus condition, int start, int count) {
        return super.selectList(NAMESPACE.concat("select_bus"), start, count,
            condition, Bus.class);
    }
